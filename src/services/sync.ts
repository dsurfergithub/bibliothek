 ...
 * toda la biblioteca viaja comprimida (gzip) y en base64url dentro del
 * fragmento `#sync=` de un enlace. Se abre el enlace en el otro dispositivo
 * y las fichas se importan ahí. El fragmento (#…) nunca llega a ningún
...
export async function buildSyncLink(cards: KnowledgeCard[]): Promise<string> {
  const json = JSON.stringify({ app: 'bibliotheke', version: 1, cards });
...
export async function decodeSyncPayload(payload: string): Promise<KnowledgeCard[]> {
  const bytes = await pipe(fromBase64Url(payload), new DecompressionStream('gzip'));
  const data = JSON.parse(new TextDecoder().decode(bytes)) as { cards?: unknown };
